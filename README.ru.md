...
3. *(опционально)* `/phase-explore 01` — исследовать кодовую базу по задачам до планирования; фиксирует паттерны, ограничения и риски
4. *(опционально)* `/impl-brief 01` — сгенерировать конкретные планы реализации по каждой задаче
5. Реализовать scope фазы (вручную, силами агента или гибрид)
6. *(опционально)* `/phase-add-task 01 "описание"` — добавить незапланированную задачу в фазу; автоматически назначает ID, выводит контракты, создаёт план
7. *(опционально)* `/impl-assist 01` — агент реализует невыполненные задачи
8. `/phase-gate 01` — запустить проверки и architect review notes
9. `/context-update 01` — завершить фазу, синхронизировать контекстные документы
10. *(опционально)* `/project-sync` — зеркалировать статусы задач в GitHub Issues + Projects Kanban-доску
