...
`sdd-workflow` is a compact workflow bundle for agent-only coding, specialized for **web
...
| Generic-agent wrapper for an integrated-project skill | `project-files/.agents/skills/<name>/SKILL.md` |
...
`/plan`, `/work`, and `/ship` are intended for integrated projects. Do not invoke them against this
repo's `docs/`. The only skill that runs from here is `/workflow-init`, and its target path must be
outside this repo.
...
2. **Keep integrated-project rules stack-neutral within the web-app scope.** The workflow assumes
   a web application (frontend + backend + database); concrete technology choices (which frontend
   framework, which backend language, which DB) belong in `project-files/docs/templates/STACK.md`,
   not `project-files/AGENTS.md`.
3. **Do not invent document files.** The shipped doc set is fixed: `SPEC.md`, `STACK.md`,
   `CHANGE_TEMPLATE.md`, `changes/NN-slug.md`, `changes/archive/NN-slug.md`, `KNOWN_GOTCHAS.md`.
   There is no `STATE.md` or `PHASE_XX.md` in the current generation — see `docs/playbooks/ship.md`
   for why archival replaced the hand-maintained contract mirror.
...
│   ├── AGENTS.md
│   ├── CLAUDE.md
│   ├── .claude/skills/<3>/SKILL.md
│   ├── .agents/skills/<3>/SKILL.md
